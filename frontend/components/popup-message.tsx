...
import { Message, useMessageContext } from '../utils/contexts/message';

const PopupMessage: NextPage = () => {
  const { messages } = useMessageContext();

  const errorPopups = (messages: Message[]) =>
    messages
      .filter((m) => m.type === 'error')
      .map((p) => (
        <div
          key={p.index}
          className="alert flex flex-row items-center bg-red-200 p-3 rounded border-b-2 border-red-300"
        >
  ...
  const successPopups = (messages: Message[]) =>
    messages
      .filter((m) => m.type === 'success')
      .map((p) => (
        <div
          key={p.index}
          className="alert flex flex-row items-center bg-green-200 p-3 rounded border-b-2 border-green-300"
        >
  ...
  return (
    <div className="absolute bottom-5 right-5">
      {errorPopups(messages)}
      {successPopups(messages)}
      {/* {successPopups(popups)} */}

      {/* <div className="alert flex flex-row items-center bg-yellow-200 p-3 rounded border-b-2 border-yellow-300">
      ...
      <div className="alert flex flex-row items-center bg-blue-200 p-3 rounded border-b-2 border-blue-300">
